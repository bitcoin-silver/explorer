...
const createLock = (cb: () => void) => {
  if (database !== "index") return cb();
  fs.writeFileSync(lockFile, process.pid.toString());
  cb();
};
const removeLock = (cb: () => void) => {
  if (database !== "index") return cb();
  fs.unlinkSync(lockFile);
  cb();
};
const isLocked = (cb: (locked: boolean) => void) => {
  if (database !== "index") return cb(false);
  cb(fs.existsSync(lockFile));
};
...
function calculateHashrate(diff: number) {
  return (diff * 2 ** 32) / 60;
}
...
async function processTransaction(db: Db, tx: any, block: any) {
  ...
  for (const vin of tx.vin) {
    ...
    inputs.push({
      addresses: out ? extractAddress(out) : "script:unknown",
      amount: out ? Number(out.value) : 0,
    });
  }

  const outputs = tx.vout
    .filter((v: any) => !(v.value === 0 && v.scriptPubKey.type === "nulldata"))
    .map((v: any) => ({
      addresses: extractAddress(v),
      amount: Number(v.value),
    }));

  const inputTotal = inputs.reduce((s, i) => s + i.amount, 0);
  const outputTotal = outputs.reduce(
    (s: any, o: { amount: any }) => s + o.amount,
    0,
  );
...
async function updateAddressesFromTx(
  ...
  for (const c of chunk(addresstxsOps, 500)) {
    try {
      await db.collection("addresstxs").bulkWrite(c, { ordered: false });
    } catch (e) {
      console.error(e);
    }
  }
  for (const c of chunk(addressesOps, 500)) {
    try {
      await db.collection("addresses").bulkWrite(c, { ordered: false });
    } catch (e) {
      console.error(e);
    }
  }
...
async function processBlock(db: Db, block: any) {
  ...
  for (const tx of block.tx) {
    const fullTx =
      typeof tx === "string" ? await rpc.getRawTransaction(tx, true) : tx;
    txids.push(fullTx.txid);
    total += fullTx.vout.reduce(
      (s: number, v: { value: any }) => s + Number(v.value),
      0,
    );
...
async function updateRichlist(db: Db) {
  const top = await db
    ...
    .limit(100)
    .toArray();
  const richlist: RichlistEntry[] = top.map((a) => ({
    address: a.a_id,
    balance: a.balance,
    received: a.received,
  }));
  await db
    ...
    .updateOne(
      ...
      { $set: { balance: richlist, received: richlist } },
      { upsert: true },
    );
}
    ...
    .updateOne(
      { coin: process.env.NEXT_PUBLIC_COIN_SYMBOL },
      {
        $set: {
          ...
          count: info.blocks,
          last: info.blocks,
          supply: supply[0]?.totalSupply || 0,
          connections: net.connections,
          difficulty: info.difficulty,
          hashrate: calculateHashrate(info.difficulty),
        },
      },
      { upsert: true },
    );
...
isLocked((locked) => {
  ...
  createLock(async () => {
    ...
    const stats = await db
      .collection("stats")
      .findOne({ coin: process.env.NEXT_PUBLIC_COIN_SYMBOL });
